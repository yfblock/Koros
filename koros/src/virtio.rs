...
use kor::{BlockDevice, BlockError};
...
unsafe impl Hal for KorosHal {
    fn dma_alloc(pages: usize, _direction: BufferDirection) -> (PhysAddr, NonNull<u8>) {
        let pa = kor_frame::alloc_frames(pages).expect("virtio dma_alloc: out of frames");
        let va = kor::arch::phys_to_virt(pa);
        // The frame allocator does not zero; `Hal` requires zeroed pages.
        unsafe {
            core::ptr::write_bytes(va as *mut u8, 0, pages * kor_frame::PAGE_SIZE);
        }
        (pa as PhysAddr, NonNull::new(va as *mut u8).unwrap())
    }

    unsafe fn dma_dealloc(paddr: PhysAddr, _vaddr: NonNull<u8>, pages: usize) -> i32 {
        kor_frame::free_frames(paddr as usize, pages);
        0
    }

    unsafe fn mmio_phys_to_virt(paddr: PhysAddr, _size: usize) -> NonNull<u8> {
        NonNull::new(kor::arch::phys_to_virt(paddr as usize) as *mut u8).unwrap()
    }
    ...
    unsafe fn share(buffer: NonNull<[u8]>, _direction: BufferDirection) -> PhysAddr {
        ...
        kor::arch::virt_to_phys(va) as PhysAddr
    }
...
pub struct VdBlk<T: Transport> {
    ...
    op_lock: kor_sched::sync::Mutex<()>,
    /// Posted by the interrupt handler on request completion.
    completion: kor_sched::Semaphore,
...
impl<T: Transport> VdBlk<T> {
    fn from_blk(blk: VirtIOBlk<KorosHal, T>) -> Self {
        ...
        Self {
            inner: Mutex::new(blk),
            capacity_sectors,
            op_lock: kor_sched::sync::Mutex::new(()),
            completion: kor_sched::Semaphore::new(0),
    ...
    fn use_irq(&self) -> bool {
        self.irq_driven.load(Ordering::Relaxed) && kor_sched::is_ready()
    }
...
use kor::driver::{DeviceDriver, DtDevice, DriverError};
...
fn wire_mmio_irq<T: Transport + 'static>(dev: &DtDevice, device: &alloc::sync::Arc<VdBlk<T>>) {
    // Route through the installed interrupt controller; on stub platforms
    // (loongarch64 extioi not wired) this returns `None` and the device
    // falls back to polling.  The controller logs the arch-specific IRQ line.
    if let Some(irq) = kor::enable_device_irq(&dev.interrupts) {
        let handler = device.clone();
        crate::registries::IRQS.register(irq, alloc::boxed::Box::new(move || handler.handle_irq()));
    ...
    fn probe(&self, dev: &DtDevice) -> Result<(), DriverError> {
        ...
        match transport.device_type() {
            DeviceType::Block => {
                ...
                crate::registries::BLOCKS.register(device);
                Ok(())
            }
...
pub fn probe_pci_ecam_and_register(ecam_base: usize, mmio_base: u64, mmio_size: u64) {
    ...
    let ecam_va = kor::arch::phys_to_virt(ecam_base);
    kor::println!("virtio-drivers: scanning PCIe ECAM at {:#x} for virtio-blk...", ecam_base);
    ...
    if next > mmio_base + mmio_size {
        kor::println!("virtio-drivers: PCIe MMIO window exhausted");
        return;
    }
    ...
    let transport = match PciTransport::new::<KorosHal, _>(&mut root, df) {
        Ok(t) => t,
        Err(e) => {
            kor::println!("virtio-drivers: PciTransport::new failed: {:?}", e);
            return;
        }
    };
    match VirtIOBlk::<KorosHal, _>::new(transport) {
        Ok(blk) => {
            ...
            crate::registries::BLOCKS.register(alloc::sync::Arc::new(VdBlk::from_blk(blk)));
        }
        Err(e) => kor::println!("virtio-drivers: VirtIOBlk init failed: {:?}", e),
    }
}
...
pub fn discover_pci_blk() -> Option<VdBlk<virtio_drivers::transport::pci::PciTransport>> {
    ...
    kor::println!("virtio-drivers: scanning PCI bus for virtio-blk...");
    ...
    match VirtIOBlk::<KorosHal, _>::new(transport) {
        ...
        Err(e) => {
            kor::println!("virtio-drivers: VirtIOBlk init failed: {:?}", e);
            None
        }
    }
}
...
pub fn probe_pci_and_register() {
    if let Some(blk) = discover_pci_blk() {
        crate::registries::BLOCKS.register(alloc::sync::Arc::new(blk));
    }
}
