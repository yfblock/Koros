...
fn reg(off: usize) -> *mut u32 {
    (crate::mm::phys_to_virt(PLIC_BASE) + off) as *mut u32
}
...
/// Handle a supervisor external interrupt: claim, dispatch, complete, looping
/// until the PLIC has no more pending sources for this hart.
pub fn handle_external() {
    let ctx = context(crate::smp::cpu_id());
    let claim = reg(0x20_0004 + ctx * 0x1000);
    loop {
